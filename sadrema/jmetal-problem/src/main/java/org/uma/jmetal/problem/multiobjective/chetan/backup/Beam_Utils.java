...
{ 
    ...
    void DFS(int M[][], int row, int col, boolean visited[][], Beam island) 
    { 
        ...
        String id = String.format("%02d", row+1) + "-" + String.format("%02d", col+1);
        GridCell cell = SDRM_BinaryMap.gridcells.get(id);
    ...
    { 
        ...
        int D[][]=  new int[][] {{4, 5, 0, 0, 0}, 
         ...
         SDRM_BinaryMap.gridcells = new HashMap<>();  						
        for(int i=0; i<ROW; i++)
        {
        	for(int j=0; j<COL; j++)
        	{
        		 String id = String.format("%02d", i) + "-" + String.format("%02d", j);
        		GridCell cell = new GridCell(id);
        		Demand demand = new Demand("User", D[i][j], 2);
        		cell.addDemand(demand);
        		SDRM_BinaryMap.gridcells.put(id, cell);
        	}
        }
        Beam_Utils I = new Beam_Utils();
        //System.out.println("Number of islands is: "+ I.countIslands(M));
        System.out.println("Beams: "+ I.countIslands(M).size());
